...
object HLists {
  ...
  final case class |:[H, T <: HList](head : H, tail : T) extends HList {
    ...
    type Head = H
    type Tail = T
    type Append[L <: HList] = HLists.|:[H, T#Append[L]]
    type ReverseAppend[L <: HList] = Tail#ReverseAppend[HLists.|:[H, L]]
  ...
  implicit def hlistConsAppender[H, T <: HList, L2 <: HList, R <: HList]
    (implicit fn : AppendFn[T, L2]) = AppendFn[HCons[H, T], L2]((l1 : HCons[H, T], l2 : L2) => HLists.|:(l1.head, fn(l1.tail, l2)))
  ...
  implicit def hlistConsReverseAppender[H, T <: HList, L2 <: HList, R <: HList]
    (implicit fn : ReverseAppendFn[T, HCons[H, L2]]) = ReverseAppendFn[HCons[H, T], L2]((l1 : HCons[H, T], l2 : L2) => fn(l1.tail, HLists.|:(l1.head, l2)))
  ...
  implicit def hlistRemoveNth[H, T <: HList, P <: Nat](implicit fn : RemoveNthFn[T, P]) = 
    RemoveNthFn[HCons[H, T], Succ[P]](l => HLists.|:(l.head, fn(l.tail)))


  // Insert nth

  implicit def hlistInsertNth0[L <: HList, E] = InsertNthFn[L, _0, E]((l, elem) => HLists.|:(elem, l))
  
  implicit def hlistInsertNth[H, T <: HList, P <: Nat, E](implicit fn : InsertNthFn[T, P, E]) = 
    InsertNthFn[HCons[H, T], Succ[P], E]((l, elem) => HLists.|:(l.head, fn(l.tail, elem)))


  // Replace by type

  implicit def hlistReplaceByType0[T <: HList, E] = ReplaceByTypeFn[HCons[E, T], _0, E]((l, elem) => HLists.|:(elem, l.tail))
  
  implicit def hlistReplaceByTypeNthMatch[T <: HList, P <: Nat, E](implicit fn : ReplaceByTypeFn[T, P, E]) = 
    ReplaceByTypeFn[HCons[E, T], Succ[P], E]((l, elem) => HLists.|:(elem, fn(l.tail, elem)))

  implicit def hlistReplaceByTypeNthNoMatch[H, T <: HList, N <: Nat, E](implicit fn : ReplaceByTypeFn[T, N, E]) = 
    ReplaceByTypeFn[HCons[H, T], N, E]((l, elem) => HLists.|:(l.head, fn(l.tail, elem)))
